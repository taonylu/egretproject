...
module flash {
    ...
    export class ApplicationDomain {
        private $parentDomain:ApplicationDomain = null;
        ...
        public getDefinition(name:string):any {
            var defName:string = SwfUtils.getDefName(this.resModule, name);
            var hasDef:boolean = this.hasDefinition(name);
            if (!hasDef) {//从swf资源里找不到定义后要从全局找

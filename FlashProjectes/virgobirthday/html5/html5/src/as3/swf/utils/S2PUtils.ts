...
module egret {
	...
	export class S2PUtils{
		...
		public static SetMatrixAndColorTransform(dis:DisplayObject, po:TransformInfo):void {
			if(null == po || null == dis){
				return;
			}
			...
			////dis.y = m.ty;
			if(null != po.color){
				var colorTF:egret.ColorTransform = new egret.ColorTransform(po.color.rm, po.color.gm, po.color.bm, po.color.am, po.color.ro, po.color.go, po.color.bo, po.color.ao);

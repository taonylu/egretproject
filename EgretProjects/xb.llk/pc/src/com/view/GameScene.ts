...
class GameScene extends BaseScene{
    ...
    //发送完成一关
    public sendLevelDone(isFinish:Boolean, mapData:any): void {
    ...
    public revPro(data): void {
        ...
        var mapData: string = data.mapData; //道具使用后影响的位置
    }
    
    //使用道具(用户)
    public revUserPro(data) {
        var status: number = data.status;      //是否使用成功 1 0
        var change: number = data.data.change; // 该道具剩余次数
    }
    ...
    public revGameOver(data): void {
        if(egret.Capabilities.isMobile) {
            var rank: number = data.rank;     //自己的排名
        } else {

...
module egret {
	...
	export class Resconfig{
		...
		public createFromObject(target:any):void{
			...
			for(var key in target){
				if("symbols" == key){
					...
					if(temp_symbols instanceof Array){//压缩形式（新写法）
						for(var i:number=0;i<temp_symbols.length;i++){
							var newSymbo:SymbolClass = new SymbolClass(temp_symbols[i]);
							this.symbols[newSymbo.className] = newSymbo;
						}
					}else{//字典形式（老写法）
					...
					for (var tagid = 0 in temp_resdef){
						...
						switch(type){
							...
							case Config.RESButton:
								var button:DefineButton = new DefineButton(tagobj);
								new_resdef[tagid] = button;
								break;
							case Config.RESSound:
								var sound:DefineSound = new DefineSound(tagobj);
		 ...
		 * @returns {*}
		 */
		public getScalingGridInfo(id:number):Array<number>
		{

module egret {
	export class SwfMovie extends SwfSprite{
		...
		public constructor(conf:Resconfig = null, symbolName:string = "", id:number = -1){
			super();
			this.init(conf, symbolName, id);
		}
		public init(conf:Resconfig = null, symbolName:string = "", id:number = -1){
		...
		private $recycleInnerObj(obj:egret.DisplayObject):void{
			...
			//	if(result) return;
			//}
			if(obj["__type_name"] == "egret.SwfSprite"){
				var result:boolean = SwfRes.Pool_recycle(SwfRes.PoolName_Common, null, obj, "egret.SwfSprite");
			}
			else if(obj["__type_name"] == "egret.Sprite"){
				var result:boolean = SwfRes.Pool_recycle(SwfRes.PoolName_Common, null, obj, "egret.Sprite");
			}
			else if(obj instanceof flash.Bitmap){
				var result:boolean = SwfRes.Pool_recycle(this.conf.path, this.conf, obj, obj['defId'], obj['extendinfo']);
				if(result) return;
			}
			else if(obj instanceof egret.SwfMovie){//动画回收后继续回收子对象
				var result:boolean = SwfRes.Pool_recycle(this.conf.path, this.conf, obj, obj['defId']);
				//if(result) return;
			}
			else if(obj instanceof SwfButton){//回收后继续回收子对象
		...
		private $addShape(def:DefineShape, parantSP:egret.Sprite, po:PlaceObject):void{
			...
			if(null != this.$curFrameObjs[po.depth]){
				...
				shapeContainer = SwfRes.Pool_getByID(SwfRes.PoolName_Common, null, "egret.Sprite");
				this.$curFrameObjs[po.depth] = shapeContainer;
				//shape png
				if(def.isexportpng){
					var shapePNG = SwfRes.Pool_getByID(this.conf.path, this.conf, def.id);
		...
		private $addSprite(defsp:DefineSprite, parantSP:SwfSprite, po:PlaceObject):void{
			...
			if(null != this.$curFrameObjs[po.depth]){
				...
				mc = SwfRes.Pool_getByID(this.conf.path, this.conf, defsp.id);
		...
		private $placeObjects(datas:Array<PlaceObject>, parantSP:SwfSprite=null, frame:number = 1):egret.Sprite{
			if(null == parantSP){
				...
				parantSP = SwfRes.Pool_getByID(SwfRes.PoolName_Common, null, "egret.SwfSprite");
			}

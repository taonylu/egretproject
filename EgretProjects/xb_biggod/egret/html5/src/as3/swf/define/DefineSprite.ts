...
module egret {
	...
	export class DefineSprite extends DefineBase{
		...
		private fromSlimData(obj:any){
			this.f = obj[2];
			this.fd = [];
			var slimFD = obj[3];
			if(slimFD && slimFD.length>0){
				for(var i:number=0;i<slimFD.length;i++){
					var pos = slimFD[i];
					var vec:Array<egret.PlaceObject> = [];
					this.fd[i] = vec;
					for(var j:number=0;j<pos.length;j++){
						var po:egret.PlaceObject = new egret.PlaceObject(pos[j]);
						vec.push(po);
					}
				}
			}

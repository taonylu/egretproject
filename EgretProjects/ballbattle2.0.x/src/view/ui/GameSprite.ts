...
class GameSprite extends egret.Sprite{
    ...
    private createPlayer(): void { 
        ...
        for(var i: number = 0;i < json.playernum;i++) {  //遍历玩家列表
...
o
    ...
    public revEatRect(): void { 
        var json = JSONManager.getInstance().revEatRect;
        var player:BaseSpore = this.playerM.getPlayerBySporeID(json.hunterid);

        if(player != null) {
            var eatList = json.eatlist;
            var len: number = eatList.length;
            var rectID: number;
            var rect: Rect;
            for(var i: number = 0;i < len;i++) {
                rectID = eatList[i];
    ...
    public revEatPlayer(): void { 
        var json = JSONManager.getInstance().revEatPlayer;
        var hunter: BaseSpore = this.playerM.getSporeBySporeID(json.hunterid);
        var food: BaseSpore = this.playerM.getSporeBySporeID(json.foodid);
    ...
    public revCreateNewRect(): void { 
        var json = JSONManager.getInstance().revCreateNewRect;
        this.generationRect(json.rectnum,json.rectlist);
    }

    public revUserLeave(): void { 
    ...
    public revEatPaoPao():void{
        var json = JSONManager.getInstance().revEatPaoPao;
        var player:BaseSpore =this.playerM.getSporeBySporeID(json.hunterid);
        var paopao:PaoPao = this.paopaoList[json.paopaoid];

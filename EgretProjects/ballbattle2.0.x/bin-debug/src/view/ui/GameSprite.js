...
var GameSprite = (function (_super) {
    ...
    __egretProto__.createHero = function () {
        var json = JSONManager.getInstance().gameData;
        var myHero = new BaseSpore();
        myHero.setName(StringTool.mixUnicodeToCh(json.hero[1]));
        myHero.setSkin(json.hero[6]);
        myHero.x = json.hero[4];
        myHero.y = json.hero[5];
        this.addChild(myHero);
        myHero.userID = json.hero[0];
        myHero.sporeID = json.hero[7];
    ...
    __egretProto__.createPlayer = function () {
        ...
        for (var i = 0; i < json.playernum; i++) {
    ...
    __egretProto__.createRect = function () {
        var json = JSONManager.getInstance().gameData;
        this.generationRect(json.rectnum, json.rectlist);
    };
    __egretProto__.generationRect = function (rectNum, rectList) {
        var rect;
    ...
    //六角方块的显示
    __egretProto__.setRectVisible = function () {
        var rect;
    ...
    __egretProto__.movePlayer = function () {
    ...
    __egretProto__.moveCamera = function () {
    ...
    __egretProto__.changeCamera = function () {
    ...
    __egretProto__.checkColloise = function () {
        this.colloiseCount++;
    ...
    __egretProto__.checkRectColloise = function () {
        var rect;
        var myHero;
        var sporeList = this.playerM.getSporeListByUserID(this.myHero.userID);
        for (var i in sporeList) {
            ...
            if (this.eatRectList.length != 0) {
                var json = JSONManager.getInstance().eatRect;
                json.cmd = NetConst.EAT_RECT;
                json.hunterid = myHero.sporeID;
                json.eatlist = this.eatRectList;
                ClientSocket.getInstance().send(json);
                this.eatRectList.length = 0;
            }
        }
    };
    __egretProto__.checkPlayerColloise = function () {
                    ...
                    if (spore.isChecked == false && mySpore.isChecked == false && spore.sporeID != mySpore.sporeID) {
                        ...
                        if (Math.abs(spore.x - mySpore.x) < hitArea && Math.abs(spore.y - mySpore.y) < hitArea) {
                            ...
                            if (spore.weight != mySpore.weight) {
                                ...
                                //发送玩家互吃
                                var eatPlayerJson = JSONManager.getInstance().eatPlayer;
                                eatPlayerJson.cmd = NetConst.EAT_PLAYER;
                                eatPlayerJson.hunterid = hunterID;
                                eatPlayerJson.foodid = foodID;
                                ClientSocket.getInstance().send(eatPlayerJson);
                            }
                        }
                    }
                }
    ...
    __egretProto__.checkPaoPaoColloise = function () {
        var paopao;
        var myHero;
        var sporelist = this.playerM.getSporeListByUserID(this.myHero.userID);
            ...
            if (!paopao.isChecked) {
                for (var j in sporelist) {
                    ...
                    if (Math.abs(paopao.x - myHero.x) < myHero.radius && Math.abs(paopao.y - myHero.y) < myHero.radius) {
                        ...
                        var json = JSONManager.getInstance().eatPaoPao;
                        json.cmd = NetConst.EAT_PAO_PAO;
                        json.hunterid = myHero.sporeID;
                        json.paopaoid = paopao.ID;
                        ClientSocket.getInstance().send(json);
                        break;
                    }
                }
    ...
    __egretProto__.startGameTimer = function () {
    ...
    __egretProto__.onGameTimerHandler = function () {
    ...
    __egretProto__.deleteAllRect = function () {
        for (var key in this.rectList) {
    ...
    __egretProto__.resetGameData = function () {
        //隐藏手柄
        this.handleUI.hide();
        //重置状态
    ...
    __egretProto__.sendPlayerMove = function (angle) {
        if (this.bSendMoveing) {
            return;
        }
        this.sendTime = egret.getTimer();
        this.bSendMoveing = true;
        var json = JSONManager.getInstance().playerMove;
        json.cmd = NetConst.MOVE_REQUEST;
        json.userid = UserManager.getInstance().userID;
        json.angle = angle.toString(); //服务端不能处理浮点数...这里转成字符串....
        var myHero;
        var myHeroList = this.playerM.getSporeListByUserID(this.myHero.userID);
        for (var i in myHeroList) {
            myHero = myHeroList[i];
            json.sporelist.push([myHero.sporeID, myHero.x.toString(), myHero.y.toString()]);
        }
        ClientSocket.getInstance().send(json);
        json.sporelist.length = 0;
    };
    __egretProto__.revMovePlayer = function () {
        console.log("lag:" + (egret.getTimer() - this.sendTime));
        var json = JSONManager.getInstance().revPlayerMove;
        var sporelist = json.sporelist;
        var localSporeList = this.playerM.getSporeListByUserID(json.userid);
        var sporeID;
        for (var i = 0; i < sporelist.length; i++) {
            ...
            if (localSporeList[sporeID]) {
                ...
                player.moveToByAngle(json.angle);
    ...
    __egretProto__.revAddPlayer = function () {
        var json = JSONManager.getInstance().playerJoin;
        //判断新玩家是否是自己。
        console.log("新玩家加入:" + json.userid);
        if (json.userid != UserManager.getInstance().userID) {
            var player = new BaseSpore();
            player.setSkin(json.skinid);
            player.setName(StringTool.mixUnicodeToCh(json.username));
            player.reset();
            player.weight = json.weight;
            player.x = json.x;
            player.y = json.y;
            player.userID = json.userid;
            player.sporeID = json.sporeid;
            this.addChild(player);
            this.playerM.addPlayer(player);
    ...
    __egretProto__.revEatRect = function () {
        var json = JSONManager.getInstance().revEatRect;
        var player = this.playerM.getPlayerBySporeID(json.hunterid);
        if (player != null) {
            var eatList = json.eatlist;
            var len = eatList.length;
            var rectID;
            var rect;
            for (var i = 0; i < len; i++) {
                rectID = eatList[i];
                ...
                this.playerM.addWeightBySporeID(json.hunterid, rect.weight);
            }
    ...
    __egretProto__.revEatPlayer = function () {
        var json = JSONManager.getInstance().revEatPlayer;
        var hunter = this.playerM.getSporeBySporeID(json.hunterid);
        var food = this.playerM.getSporeBySporeID(json.foodid);
        ...
        food.die();
        this.playerM.addWeightBySporeID(json.hunterid, food.weight);
        this.playerM.reduceWeightBySporeID(json.foodid, food.weight);
        this.playerM.deleteSpore(json.foodid);
        //自己被吃掉
    ...
    __egretProto__.revCreateNewRect = function () {
        var json = JSONManager.getInstance().revCreateNewRect;
        this.generationRect(json.rectnum, json.rectlist);
    };
    __egretProto__.revUserLeave = function () {
        var json = JSONManager.getInstance().revUserLeave;
        var sporeList = this.playerM.getSporeListByUserID(json.userid);
        if (sporeList == null) {
        ...
        this.playerM.deleteUser(json.userid);
    };
    __egretProto__.sendTuPaoPao = function () {
        var myWeight = this.playerM.getWeightByUserID(this.myHero.userID);
        if (myWeight >= GameConst.tuPaoPaoWeightLimit) {
            var json = JSONManager.getInstance().tuPaoPao;
            json.cmd = NetConst.TU_PAO_PAO;
            json.userid = this.myHero.userID;
            ClientSocket.getInstance().send(json);
        }
    };
    __egretProto__.revTuPaoPao = function () {
        var json = JSONManager.getInstance().revTuPaoPao;
        var localScopeList = this.playerM.getSporeListByUserID(json.userid);
        var paopaoList = json.paopaolist;
        var spore;
        var paopao;
        ...
        for (var key in paopaoList) {
            ...
            if (spore.weight >= GameConst.tuPaoPaoWeightLimit) {
                ...
                paopao.ID = json.paopaolist[key][1];
                this.paopaoList[paopao.ID] = paopao;
                this.playerM.reduceWeightBySporeID(spore.sporeID, GameConst.paopaoWeight);
            }
        }
        if (json.userid == this.myHero.userID) {
            this.gameScene.setWeightLabel(this.playerM.getWeightByUserID(this.myHero.userID));
        }
    };
    __egretProto__.revEatPaoPao = function () {
        var json = JSONManager.getInstance().revEatPaoPao;
        var player = this.playerM.getSporeBySporeID(json.hunterid);
        var paopao = this.paopaoList[json.paopaoid];
        if (player == null) {
            console.log("error: revEatpaopao player is null");
        }
        player.eat(paopao.weight);
        paopao.hide();
        delete this.paopaoList[json.paopaoid];
        this.playerM.addWeightBySporeID(player.sporeID, paopao.weight);
        if (player.userID = this.myHero.userID) {
            this.gameScene.setWeightLabel(this.playerM.getWeightByUserID(this.myHero.userID));
        }
    };
    __egretProto__.sendFenLie = function () {
        if (this.myHero.weight >= GameConst.fenlieLimit) {
            var sporeList = this.playerM.getSporeListByUserID(this.myHero.userID);
            //当前孢子数量小于一定数量时，才能分裂
            if (ArrayTool.getObjectLen(sporeList) <= GameConst.fenlieMax) {
                var json = JSONManager.getInstance().fenLie;
                json.cmd = NetConst.FEN_LIE;
                json.userid = this.myHero.userID;
                ClientSocket.getInstance().send(json);
            }
        }
    };
    __egretProto__.revFenLie = function () {
        var json = JSONManager.getInstance().revFenLie;
        var sporeList = json.sporelist;
        var localSporeList = this.playerM.getSporeListByUserID(json.userid);
        var player;
